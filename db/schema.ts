...
// In-app notifications, generated by the scheduled reminder job. Kept as records
// (not just on-the-fly banners) so a reminder fires once, carries a read state,
// and can later be delivered by Web Push.
export const notifications = pgTable("notifications", {
  id: uuid("id").primaryKey().defaultRandom(),
  userId: uuid("user_id")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
  // return_due_soon | return_overdue (more kinds later)
